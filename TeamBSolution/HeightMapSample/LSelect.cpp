...
bool LSelect::GetIntersection(TVector3 vStart, TVector3 vEnd, TVector3 vNormal, 
								TVector3 v0, TVector3 v1, TVector3 v2, float* fPercentage)
{
...
bool LSelect::MapIsIntersection(LHeightMap* map)
{
	TVector3 v0, v1, v2;
	...
	//for  ( int iFace = 0; iFace < list.size() / 3; iFace++)
	//{
	//	v0 = list[iFace * 3 + 0].p;
	//	v0 = list[iFace * 3 + 1].p;
	//	v0 = list[iFace * 3 + 2].p;
			...
			v0, v1, v2) && LInput::GetInstance().m_dwKeyState[VK_LBUTTON])
		{
			if (PointInPolygon(m_vIntersection, vNormal, v0, v1, v2))
			{
				std::wstring text = L"Click to Face";
				LWrite::GetInstance().AddText(text, 0.0f, 100.0f, { 1.0f, 1.0f, 1.0f, 1.0f });
				//list[map->m_IndexList[iFace * 3 + 0]].p.y += 100;
				return true;
