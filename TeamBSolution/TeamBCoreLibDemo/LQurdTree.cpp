...
LVector2 LQurdtree::GetHeightFormNode(LNode* pNode)
{
...
void LQurdtree::ComputeBoundingBox(LNode* pNode)
{
	...
	LVector2 vHeight = GetHeightFormNode(pNode);
...
void LQurdtree::UpdateIndexBuffer()
{
	...
	HRESULT hr = m_pMap->m_pImmediateContext->Map(m_pIndexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (SUCCEEDED(hr))
	{
		memcpy(mappedResource.pData, &m_IndexList.at(0), sizeof(DWORD) * m_IndexList.size());
		m_pMap->m_pImmediateContext->Unmap(m_pIndexBuffer, 0);
	}
}
