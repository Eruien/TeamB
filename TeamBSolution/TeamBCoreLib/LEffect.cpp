...
bool LEffect::Frame()
{
	if (LInput::GetInstance().m_dwKeyState['P'] == DWORD(KeyState::KEY_PUSH))
	{
		...
		Spliteinfo.m_vPos.z = -500.0f;//z위치 랜덤생성 실사용시 입력받으면됨
		...
		if (Spliteinfo.m_iMaxFrame == 1)
		{
			Spliteinfo.m_vSclae = { (float)LGlobal::g_WindowWidth,(float)LGlobal::g_WindowHeight,1.0f };
