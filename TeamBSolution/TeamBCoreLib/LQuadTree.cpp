...
void LQuadtree::SetDepth(int depth)
{
	m_TreeDepth = depth;
}

LNode* LQuadtree::NewNode(LNode* pParnet, DWORD topLeft, DWORD topRight, DWORD bottomRight, DWORD bottomLeft)
{
...
void LQuadtree::BuildTree(LNode* pNode)
{
	...
	LNode* newNode = NewNode(pNode,
		...
		Center,
		LeftCenter);
	pNode->childNodeList.push_back(newNode);
	...
	{
		BuildTree(pNode->childNodeList[i]);
	}
}

void LQuadtree::BuildQuadTree(LMap* pMap, DWORD row, DWORD col)
{
	m_pMap = pMap;
	m_Row = row;
	m_Col = col;
...
TVector2 LQuadtree::GetHeightFormNode(LNode* pNode)
{
...
void LQuadtree::ComputeBoundingBox(LNode* pNode)
{
...
void LQuadtree::FindNode(LNode* pNode)
{
	...
	do
	{
		for (int i = 0; i < pNode->childNodeList.size(); i++)
		{
			if (pNode->childNodeList[i] != nullptr)
			{
				if (CullResult::INSIDE == LGlobal::g_pMainCamera->m_Frustum.CheckOBBInPlane(pNode->childNodeList[i]->m_tBox))
				{
					m_NodeList.push_back(pNode->childNodeList[i]);
					continue;
				}
				m_Queue.push(pNode->childNodeList[i]);
...
void LQuadtree::AddLeafNode(LNode* pNode)
{
	if (pNode->m_IsLeaf)
	{
	...
	for (int i = 0; i < pNode->childNodeList.size(); i++)
	{
		AddLeafNode(pNode->childNodeList[i]);
	}
}

void LQuadtree::UpdateIndexBuffer()
{
