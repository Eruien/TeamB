...
//     a                    b                       c
bool LSelect::IntersectRayToSphere(T_SPHERE* pSphere, T_RAY* pRay)
{
...
bool LSelect::IntersectBox(T_BOX* pBox, T_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	TVector3 tmin;
...
bool LSelect::AABBtoRay(T_BOX* a, T_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	float t_min = 0;
...
bool LSelect::OBBtoRay(T_BOX* pBox, T_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
...
bool LSelect::ChkOBBToRay(T_BOX* pBox, T_RAY* pRay)
{
	if (pRay == NULL) pRay = &m_Ray;
	float t_min = -999999.0f;
	float t_max = 999999.0f;
...
bool LSelect::ChkPick(TVector3& v0, TVector3& v1, TVector3& v2)
{
	FLOAT fBary1, fBary2;
	FLOAT fDist;
...
bool LSelect::IntersectTriangle(const TVector3& orig, const TVector3& dir,
