...
bool Sample::Init()
{
	...
	m_DebugCamera->CreateLookAt({ 0.0f, 10.0f, -10.0f }, { 0.0f, 0.0f, 1.0f });
	...
	//fbxObj = LFbxMgr::GetInstance().Load(L"../../res/fbx/man/man.fbx");
	...
	m_Triangle = new LObject;
	m_Triangle->Set();
 	m_Triangle->Create(L"../../res/hlsl/CustomizeMap.hlsl", L"../../res/map/baseColor.jpg");
...
bool Sample::Frame()
{
	...
	m_Select->ObjectIsIntersection(m_Triangle);
...
bool Sample::Render()
{
	...
	m_Triangle->SetMatrix(nullptr, &LGlobal::g_pMainCamera->m_matView, &LGlobal::g_pMainCamera->m_matProj);
	m_Triangle->Render();
	return true;
}
