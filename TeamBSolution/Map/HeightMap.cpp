...
{
    return (n & (n - 1)) == 0; // 2의 거듭제곱은 10000...0 의 형태이다. 따라서 n과 n-1의 and비트연산을 하면 무조건 0이 나와야 한다.
}
...
int HeightMap::ResizeMap(int n)
{
    int result = 0;
    int N = 2;
    while (N < n) // N이 n보다 클 때 까지 2를 곱한다. 그러나 n보다 이미 크다면 반환.
    {
        result = N;
        N *= 2;
    }
...
void HeightMap::Init(UINT& rowNum, UINT& colNum, float heightScale, wstring heightTexPath)
{
    heightMapTexture = make_unique<Texture>();
    if (heightMapTexture->Load(heightTexPath))
    {
        TVector2 size = heightMapTexture->GetSize();
        ...
        if (!CheckSquare(mData.height - 1))
            mData.height = ResizeMap(mData.height); // 이 두 함수는 직접 객체 포인터를 전달해서 값을 변경하는 것도 가능하겠음.

        this->rowNum = rowNum = mData.height;       // this 는 heightmap
...
void HeightMap::SaveHeightMap(wstring savePath)
{
    vector<BYTE> heightListByte;
...
float HeightMap::GetHeightByIndex(UINT index)
{
