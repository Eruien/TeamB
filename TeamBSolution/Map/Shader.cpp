...
Shader::Shader(std::wstring file) : file(L"..\\..\\Shader\\" + file)
{
	...
	{
		LGlobal::g_pImmediateContext->RSGetState(initialStateBlock->RSRasterizerState.GetAddressOf());
		LGlobal::g_pImmediateContext->OMGetBlendState(initialStateBlock->OMBlendState.GetAddressOf(), initialStateBlock->OMBlendFactor, &initialStateBlock->OMSampleMask);
		LGlobal::g_pImmediateContext->OMGetDepthStencilState(initialStateBlock->OMDepthStencilState.GetAddressOf(), &initialStateBlock->OMStencilRef);
	}
...
ComPtr<ID3D11InputLayout> Shader::CreateInputLayout(ComPtr<ID3DBlob> fxBlob, D3DX11_EFFECT_SHADER_DESC* effectVsDesc, std::vector<D3D11_SIGNATURE_PARAMETER_DESC>& params)
{
	...
	if (inputLayoutDesc.size() > 0)
	{
		...
		HRESULT hr = LGlobal::g_pDevice->CreateInputLayout
		(
			&inputLayoutDesc[0]
